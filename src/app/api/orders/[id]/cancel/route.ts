...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try { const { id } = await params; if (!verifyOrderCapability(id, getOrderCapability(req))) return NextResponse.json({ error: 'Order not found' }, { status: 404 }); await cancelOrder(id); return NextResponse.json({ success: true }) }
  catch (err) {
