...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try { const session = await requireSession(); const { id } = await params; const { status } = await req.json(); await updateOrderStatus(id, status, session.organizationId); return NextResponse.json({ success: true }) }
