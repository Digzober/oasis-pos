...
export async function GET(request: NextRequest) {
  try {
    ...
    const result = await listLookupPaginated('producers', session.organizationId, {
      page,
      limit,
      includeInactive,
    })
    ...
    logger.error('Producers error', { error: String(err) })
...
export async function POST(request: NextRequest) {
  try {
    ...
    const producer = await createLookup('producers', { ...body, organization_id: session.organizationId })
    ...
    logger.error('Producer create error', { error: String(err) })
    return NextResponse.json({ error: 'Server error' }, { status: 500 })
  }
}
