    ...
    <div className="space-y-6">
        ...
        <table className="w-full text-sm">
          ...
          <tbody>{zones.map((z: AnyR) => (
            <tr key={z.id} className="border-b border-edge/50">
              ...
              <td className="py-2 text-right text-secondary">{z.estimated_delivery_minutes ?? 45} min</td>
            </tr>
          ))}</tbody>
        </table>
      </div>
      ...
      <div className="grid grid-cols-2 gap-6">
        <div className="bg-surface rounded-xl border border-edge p-4">
          ...
          {vehicles.map((v: AnyR) => <div key={v.id} className="text-sm text-primary py-1">{v.name} — {v.license_plate}</div>)}
        ...
        <div className="bg-surface rounded-xl border border-edge p-4">
          ...
          {drivers.map((d: AnyR) => <div key={d.id} className="text-sm text-primary py-1">{d.employees?.first_name} {d.employees?.last_name}</div>)}
