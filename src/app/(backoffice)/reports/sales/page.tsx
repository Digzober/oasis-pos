  ...
  return (
    <div>
        ...
        <div className="flex gap-2 items-center">
          ...
          {PRESETS.map((p) => (
            <button
              ...
              className="text-xs px-3 py-1.5 bg-raised text-secondary rounded-lg hover:bg-raised transition-colors"
            >
              {p.label}
            </button>
          ))}
      ...
      {loading && !summary ? (
        ...
        <div className="text-center text-muted py-12">No data</div>
      ) : (
        <>
          {/* Summary Cards */}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
            ...
            <div className="bg-surface rounded-xl border border-edge p-4">
              <h3 className="text-sm font-semibold text-secondary mb-3">Sales by Hour</h3>
              <SalesByHourChart data={summary.sales_by_hour} />
            </div>

            {/* Sales by Category */}
            <div className="bg-surface rounded-xl border border-edge p-4">
              <h3 className="text-sm font-semibold text-secondary mb-3">Sales by Category</h3>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {/* Top Products */}
            <div className="bg-surface rounded-xl border border-edge p-4">
              <h3 className="text-sm font-semibold text-secondary mb-3">Top Products</h3>
              <table className="w-full text-sm">
                ...
                <tbody>
                  {summary.top_products.map((p) => (
                    <tr key={p.product_name} className="border-b border-edge/50">
                      <td className="py-2 text-primary truncate max-w-[200px]">{p.product_name}</td>
                      <td className="py-2 text-right text-secondary tabular-nums">{p.units_sold}</td>
                      <td className="py-2 text-right text-primary tabular-nums">{fmt(p.revenue)}</td>
                    </tr>
                  ))}
                  {summary.top_products.length === 0 && (
                    <tr><td colSpan={3} className="py-4 text-center text-muted">No sales data</td></tr>
                  )}
                </tbody>
              </table>
            </div>
            ...
            {/* Sales by Employee */}
            <div className="bg-surface rounded-xl border border-edge p-4">
              <h3 className="text-sm font-semibold text-secondary mb-3">Sales by Employee</h3>
              <table className="w-full text-sm">
                <thead>
                  <tr className="text-secondary text-xs uppercase border-b border-edge">
                ...
                <tbody>
                  {summary.sales_by_employee.map((e) => (
                    <tr key={e.employee_name} className="border-b border-edge/50">
                      <td className="py-2 text-primary">{e.employee_name}</td>
                      <td className="py-2 text-right text-secondary tabular-nums">{e.count}</td>
                      <td className="py-2 text-right text-primary tabular-nums">{fmt(e.total)}</td>
                      <td className="py-2 text-right text-secondary tabular-nums">{fmt(e.average)}</td>
                    </tr>
                  ))}
                  {summary.sales_by_employee.length === 0 && (
                    <tr><td colSpan={4} className="py-4 text-center text-muted">No sales data</td></tr>
                  )}
                </tbody>
              </table>
            </div>
