...
export default function ReturnsPage() {
  ...
  const fetchData = useCallback(async () => {
    const res = await fetch('/api/registers/configure/transaction-reasons?reason_type=return', { cache: 'no-store' })
    if (res.ok) {
  ...
  const addReason = async () => {
    ...
    const res = await fetch('/api/registers/configure/transaction-reasons', {
      ...
      body: JSON.stringify({ name: newReason.trim(), reason_type: 'return' }),
    })
    if (res.ok) {
      setNewReason('')
      await fetchData()
  ...
  return (
    <div>
      ...
      <h1 className="text-xl font-bold text-gray-50 mb-6">Return Reasons</h1>
      ...
      {loading ? (
        ...
        <>
          <div className="flex flex-wrap gap-2 mb-6">
            {reasons.length === 0 ? (
              <p className="text-sm text-gray-500">No return reasons configured</p>
            ) : reasons.map(reason => (
