...
export default function VoidsPage() {
  ...
  const fetchData = useCallback(async () => {
    const res = await fetch('/api/registers/configure/transaction-reasons?reason_type=void', { cache: 'no-store' })
    if (res.ok) {
  ...
  const addReason = async () => {
    ...
    const res = await fetch('/api/registers/configure/transaction-reasons', {
      ...
      body: JSON.stringify({ name: newReason.trim(), reason_type: 'void' }),
    })
    if (res.ok) {
      setNewReason('')
      await fetchData()
  ...
  return (
    <div>
      ...
      <h1 className="text-xl font-bold text-gray-50 mb-6">Void Reasons</h1>
      ...
      {loading ? (
        ...
        <>
          <div className="flex flex-wrap gap-2 mb-6">
            {reasons.length === 0 ? (
              <p className="text-sm text-gray-500">No void reasons configured</p>
            ) : reasons.map(reason => (
