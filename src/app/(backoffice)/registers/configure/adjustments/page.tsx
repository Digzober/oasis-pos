...
export default function AdjustmentsPage() {
  ...
  const fetchData = useCallback(async () => {
    const res = await fetch('/api/registers/configure/transaction-reasons?reason_type=adjustment', { cache: 'no-store' })
    if (res.ok) {
  ...
  const addReason = async () => {
    ...
    const res = await fetch('/api/registers/configure/transaction-reasons', {
      ...
      body: JSON.stringify({ name: newReason.trim(), reason_type: 'adjustment' }),
    })
    if (res.ok) {
      setNewReason('')
      await fetchData()
