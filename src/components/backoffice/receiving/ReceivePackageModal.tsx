...
export function ReceivePackageModal({
  ...
  return (
    <div className="fixed inset-0 bg-bg/70 z-50 flex items-start justify-center overflow-y-auto pt-8 pb-8">
      <div
        className="bg-surface border border-edge rounded-2xl w-full max-w-3xl shadow-2xl"
        onKeyDown={(e) => { if (e.key === 'Escape') onClose() }}
      >
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-edge">
          <div>
            <h2 className="text-lg font-semibold text-primary">Receive Package</h2>
            <p className="text-xs text-muted mt-0.5">Item {index + 1} - {draft.barcode}</p>
          </div>
          <button type="button" onClick={onClose} className="text-secondary hover:text-primary text-xl leading-none">
            &#10005;
          </button>
        </div>
        ...
        <div className="px-6 py-4 space-y-5 max-h-[calc(100vh-200px)] overflow-y-auto">
          ...
          <section>
            <h3 className="text-xs font-semibold text-secondary uppercase tracking-wide mb-3">Product Matching</h3>
            <div className="space-y-3">
              <div>
                <span className="text-[11px] text-muted block mb-1">Listed product (BioTrack)</span>
                <p className="text-sm text-secondary bg-bg rounded-lg px-3 py-2 border border-edge">
                  {draft.biotrack_name} &mdash; {draft.original_quantity} {draft.item_type === 'weight' ? 'g' : 'qty'}
                </p>
              </div>
              <div>
                <span className="text-[11px] text-muted block mb-1">Catalog Product *</span>
                <ProductMatcher
          ...
          <section>
            <h3 className="text-xs font-semibold text-secondary uppercase tracking-wide mb-3">Quantities & Classification</h3>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
              <div>
                <span className="text-[11px] text-muted block mb-1">Tags</span>
                <div className="flex flex-wrap gap-1">
                  {tags.length > 0 ? (
                    ...
                    <span className="text-xs text-muted">No tags available</span>
                  )}
                </div>
              </div>
              <div>
                <span className="text-[11px] text-muted block mb-1">Type</span>
                <input value={draft.item_type === 'weight' ? 'Weight' : 'Quantity'} readOnly className={readOnlyCls} />
              </div>
              <div>
                <span className="text-[11px] text-muted block mb-1">Quantity *</span>
                <div className="flex gap-1">
                  <input
                    ...
                    className={inputCls}
                  />
                  <span className="flex items-center text-xs text-muted px-2 bg-bg border border-edge-strong rounded-lg">
                    {draft.item_type === 'weight' ? 'g' : 'qty'}
                  </span>
                </div>
              </div>
              <div>
                <span className="text-[11px] text-muted block mb-1">Vendor *</span>
                <SearchableSelect
                  options={vendors}
              ...
              <div>
                <span className="text-[11px] text-muted block mb-1">Grams / Concentration</span>
                <div className="flex gap-1">
                  <input
                    type="number"
                    step="0.001"
              ...
              <div>
                <span className="text-[11px] text-muted block mb-1">Producer</span>
                <SearchableSelect
                  options={vendors}
          ...
          <section>
            <h3 className="text-xs font-semibold text-secondary uppercase tracking-wide mb-3">Location & Status</h3>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
              <div>
                <span className="text-[11px] text-muted block mb-1">Room *</span>
                <SearchableSelect
                  options={rooms}
                  value={draft.room_id}
              ...
              <div>
                <span className="text-[11px] text-muted block mb-1">Subroom</span>
                <SearchableSelect
                  options={subrooms}
              ...
              <div>
                <span className="text-[11px] text-muted block mb-1">Flower Equivalent</span>
                <input
                  type="number"
                  step="0.001"
              ...
              <div>
                <span className="text-[11px] text-muted block mb-1">Expiration Date</span>
                <input
                  type="date"
              ...
              <div>
                <span className="text-[11px] text-muted block mb-1">Inventory Status</span>
                <select
          ...
          <section>
            <h3 className="text-xs font-semibold text-secondary uppercase tracking-wide mb-3">Package & Batch</h3>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
              <div>
                <span className="text-[11px] text-muted block mb-1">Package ID *</span>
                <input value={draft.barcode} readOnly className={readOnlyCls} />
              </div>
              <div>
                <span className="text-[11px] text-muted block mb-1">Packaging Date</span>
                <input
                  type="date"
              ...
              <div>
                <span className="text-[11px] text-muted block mb-1">External Package ID</span>
                <input
              ...
              <div>
                <span className="text-[11px] text-muted block mb-1">Lot Name / Batch ID *</span>
                <input
              ...
              <div>
                <span className="text-[11px] text-muted block mb-1">Package NDC</span>
                <input
          ...
          {/* Section 5: Pricing */}
          <section>
            <h3 className="text-xs font-semibold text-secondary uppercase tracking-wide mb-3">Pricing</h3>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
              <div>
                <span className="text-[11px] text-muted block mb-1">Strain</span>
                <SearchableSelect
                  options={strains}
              ...
              <div>
                <span className="text-[11px] text-muted block mb-1">Tax per Unit</span>
                <input
                  type="number"
                  step="0.01"
              ...
              <div>
                <span className="text-[11px] text-muted block mb-1">Cost per Unit *</span>
                <input
                  type="number"
                  step="0.01"
              ...
              <div>
                <span className="text-[11px] text-muted block mb-1">Med Price per Unit</span>
                <input
                  type="number"
                  step="0.01"
              ...
              <div>
                <span className="text-[11px] text-muted block mb-1">Rec Price per Unit</span>
                <input
                  type="number"
                  step="0.01"
              ...
              <div>
                <span className="text-[11px] text-muted block mb-1">Total Package Cost *</span>
                <input
                  type="number"
          ...
          <section>
            <h3 className="text-xs font-semibold text-secondary uppercase tracking-wide mb-3">Lab Results</h3>
            <div className="space-y-3">
              <div className="flex items-center gap-3">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    ...
                    className="w-4 h-4 rounded border-edge-strong bg-bg text-accent focus:ring-accent"
                  />
                  <span className="text-sm text-secondary">Get BioTrack lab results</span>
                </label>
              </div>

              <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                <div>
                  <span className="text-[11px] text-muted block mb-1">THC %</span>
                  <input
                    type="number"
                    step="0.01"
                  ...
                  {draft.use_biotrack_lab && draft.thc_percentage !== null && (
                    <span className="text-[10px] text-accent mt-0.5 block">From BioTrack</span>
                  )}
                </div>
                <div>
                  <span className="text-[11px] text-muted block mb-1">CBD %</span>
                  <input
                    type="number"
                    step="0.01"
              ...
              {draft.lab_results && Object.keys(draft.lab_results).length > 0 && (
                <details className="group">
                  <summary className="text-xs text-secondary cursor-pointer hover:text-secondary">
                    View full lab results JSON ({Object.keys(draft.lab_results).length} fields)
                  </summary>
                  <pre className="mt-2 p-3 bg-bg border border-edge rounded-lg text-xs text-secondary overflow-x-auto max-h-48">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-t border-edge">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              type="button"
              onClick={handleSave}
              className="px-5 py-1.5 bg-accent text-primary rounded-lg text-sm font-medium hover:bg-accent"
            >
              Save package
            </button>
          </div>
