...
export default function VoidTransactionPanel({ onClose }: { onClose: () => void }) {
    ...
    return (
      <div className="fixed inset-0 bg-bg/80 z-50 flex items-center justify-center">
        <div className="bg-surface rounded-2xl p-8 max-w-md w-full text-center">
          ...
          <h2 className="text-xl font-bold text-primary mb-2">Transaction Voided</h2>
          <p className="text-secondary text-sm mb-4">#{tx?.transaction_number} — {fmt(tx?.total ?? 0)} reversed</p>
  ...
  return (
    <div className="fixed inset-0 bg-bg/80 z-50 flex items-center justify-center">
      <div className="bg-surface border border-edge rounded-2xl max-w-lg w-full shadow-2xl">
        <div className="px-6 pt-5 pb-4 border-b border-edge flex items-center justify-between">
          <h2 className="text-lg font-bold text-danger">Void Transaction</h2>
        ...
        <div className="px-6 py-4 space-y-4">
          ...
          <div className="flex gap-2">
            <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}
              ...
              placeholder="Transaction number..." className="flex-1 h-11 px-4 bg-bg border border-edge-strong rounded-lg text-primary text-sm focus:outline-none focus:ring-2 focus:ring-danger" />
          ...
          {tx && (
            <div className="bg-bg rounded-lg p-4">
              <div className="flex justify-between text-sm mb-2">
                <span className="text-primary font-bold">#{tx.transaction_number}</span>
                <span className="text-primary font-bold tabular-nums">{fmt(tx.total)}</span>
              </div>
          ...
          {tx && (
            <div>
              <label className="text-sm text-secondary block mb-1">Void Reason *</label>
              <input type="text" value={reason} onChange={(e) => setReason(e.target.value)}
                placeholder="e.g., Customer changed mind" className="w-full h-11 px-4 bg-bg border border-edge-strong rounded-lg text-primary text-sm focus:outline-none focus:ring-2 focus:ring-danger" />
            </div>
          )}
        ...
        {tx && (
          <div className="px-6 py-4 border-t border-edge flex gap-3">
            ...
            <button onClick={handleVoid} disabled={!reason.trim() || isProcessing}
              className="flex-1 h-11 bg-danger text-primary rounded-lg font-bold hover:bg-danger disabled:opacity-40 disabled:cursor-not-allowed transition-colors">
